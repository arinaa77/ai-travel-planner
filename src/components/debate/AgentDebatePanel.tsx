...
function AgentCard({ agent }: { agent: AgentOutput }) {
  ...
  const ringClass = isRunning
    ? `ring-2 ring-offset-1 ${accent.dot.replace("bg-", "ring-")}`
    : isError
  ...
  return (
    <div
      className={`bg-white border border-gray-200 border-t-4 rounded-xl overflow-hidden ${accent.border} ${ringClass}`}
    >
      {/* Header */}
            ...
            {(agent.id === "budget" ? agent.items.slice(-1) : agent.items).map((item, i, arr) => {
              ...
              return (
                <li key={i} className="flex items-baseline justify-between gap-4 py-2.5">
                  <span
                    className={`text-sm truncate ${highlight ? "font-semibold text-gray-800" : "text-gray-400"}`}
                  >
                    {item.label}
                  </span>
                  <span
                    className={`text-sm font-semibold shrink-0 truncate ${highlight ? accent.value : "text-gray-800"}`}
                  >
                    {item.value}
                  </span>
                </li>
